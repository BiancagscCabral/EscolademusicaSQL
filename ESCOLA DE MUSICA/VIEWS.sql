...
ORDER BY 
    ...
    -- View de Orquestras Completas --
    CREATE OR REPLACE VIEW vw_orquestras_completas AS
SELECT 
    o.id,
    o.nome,
    o.cidade,
    o.país,
    COUNT(DISTINCT m.id_musico) AS total_musicos,
    COUNT(DISTINCT s.id_sinfonia) AS total_sinfonias,
    CONCAT('R$ ', FORMAT(SUM(i.valor), 2)) AS valor_instrumentos,
    AVG(TIMESTAMPDIFF(YEAR, m.datanasc, CURDATE())) AS idade_media_musicos
FROM Orquestra o
LEFT JOIN Musicos m ON o.id = m.Orquestra_id
LEFT JOIN Orquestra_Sinfonica os ON o.id = os.id_orquestra
LEFT JOIN Sinfonias s ON os.id_sinfonia = s.id_sinfonia
LEFT JOIN Instrumentos i ON m.id_musico = i.id_musico
GROUP BY o.id;


-- View de Instrumentos por Valor e Tipo --
CREATE OR REPLACE VIEW vw_instrumentos_detalhados AS
SELECT 
    i.id_instrumento,
    i.desc AS instrumento,
    ti.categoria,
    ti.subcategoria,
    CONCAT('R$ ', FORMAT(i.valor, 2)) AS valor,
    m.nome AS musico,
    o.nome AS orquestra
FROM Instrumentos i
JOIN TipoInstrumento ti ON i.id_tipo = ti.id_tipo
LEFT JOIN Musicos m ON i.id_musico = m.id_musico
LEFT JOIN Orquestra o ON m.Orquestra_id = o.id
